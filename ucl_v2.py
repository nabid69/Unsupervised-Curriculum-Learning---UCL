...
def make_clusters(features,
    ...
    print(f"Clustering features into {n_clusters} clusters...", end="")
...
"""
    ...
    while iteration <= stop_iter:
        ...
        pseudo_labels.append(clust.labels_)  # predicted "labels", ie clusters
        ...
        # print("DATA TYPES: centers, features \n")
        # print(type(centers))
        # print(type(features))
        ...
        # Selects the "best" samples, ie the samples closest to the centroid, from each cluster, putting
        ...
        plt.savefig(plots_path+f'it{iteration}_kmeans_'+str(iteration)+'.png', dpi=300, bbox_inches='tight')
        plt.clf()
        ...
        # probs = model.predict(ext_datagen.flow_from_dataframe(pd.DataFrame({"path": all_reliable["path"], "label": [""] * len(all_reliable["path"])}),
                                                          # x_col="path",
...
#         preds_df = pd.DataFrame({"path": all_reliable["path"],
#                                  "plabel": preds})
#         preds_df["plabel"] = preds_df["plabel"].astype(str)
        
#         print(all_reliable["plabel"])
#         print(preds_df["plabel"])
...
#     print(ary)
#     fix_idx = np.arange(len(ary))
#     labels = []
#     n =np.copy(ary)
#     assign_labels(labels, fix_idx, n)
#     print("Here I am")
#     print(labels)
#     new_preds = np.array(preds)
#     for label in labels:
#             indexes = np.where(np.array(preds)==label[0])
#             for idx in indexes:
#                 new_preds[idx] = label[1]
#     return new_preds, labels, n
...
# def assign_labels(labels, fix_idx, n):
#     if len(fix_idx) > 1:
#         tmp = np.copy(n[:, fix_idx])
#         tmp = np.copy(tmp[fix_idx, :])
#         confused_r_idx = []
#         for i, row in enumerate(tmp):
#             r_idx = np.argmax(row)
#             c_idx = np.argmax(tmp[:,r_idx])
#             # r_idx = np.argmax(tmp[i])
#             # c_idx = np.argmax(tmp[:,r_idx])
#             if i == c_idx:
#                 print([fix_idx[r_idx],fix_idx[c_idx]])
#                 labels.append(tuple([fix_idx[r_idx],fix_idx[c_idx]]))
#                 n[:, [fix_idx[c_idx],fix_idx[r_idx]]] = n[:, [fix_idx[r_idx],fix_idx[c_idx]]]
#                 print(n)
#             else:
#                 print("Confusion!!")
#                 confused_r_idx.append(i)
#         print(confused_r_idx)
#         assign_labels(labels, confused_r_idx, n)     
#     else:
#         return n
